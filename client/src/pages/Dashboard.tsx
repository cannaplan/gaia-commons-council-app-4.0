/* Updated Dashboard.tsx
   - Reorganized into 7 top-level CollapsibleCard sections as requested.
   - Added <CollapseAllToggle className="mb-6" /> immediately after hero stats.
   - Each CollapsibleCard has defaultOpen={false} and children wrapped in <div className="space-y-6 pt-2">.
   - Replaced numbered school placeholders when rendering school names with:
       /^School\s*\d+/i ? 'Sites to be confirmed upon program launch' : school.name
   - Moved the Leech Lake Tribal Programs allocation card into the Tribal Partnerships CollapsibleCard.
   - Preserved all existing classNames, animations, Framer Motion, Recharts, and Tailwind styles.
   NOTE: This file is a direct replacement for the original client/src/pages/Dashboard.tsx
*/
...
export default function Dashboard() {
  ...
  // Helper to render school display name with replacement for numbered placeholders
  const renderSchoolDisplayName = (name: string) =>
    (/^School\s*\d+/i).test(name) ? 'Sites to be confirmed upon program launch' : name;
    ...
    <CollapsibleProvider>
      <div className="min-h-screen bg-gradient-to-br from-background to-secondary/30 pb-20 print:bg-white">
        <div id="main-content" ref={dashboardRef} className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 2xl:px-12 pt-6">
          ...
          {/* STEP 1: CollapseAllToggle immediately after the hero stats */}
          <div>
            <CollapseAllToggle className="mb-6" />
          </div>

          {/* === TOP-LEVEL COLLAPSIBLE CARDS === */}

          {/* CARD 1: Pilot Program */}
          <CollapsibleCard title="🌱 Pilot Program" accent="emerald" badge="Minnesota 2026" defaultOpen={false}>
            <div className="space-y-6 pt-2">
              {/* Minnesota Pilot School Clusters section (moved here) */}
              {clusters && clusters.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="mb-0">
                                ...
                                {clusterSchools.map(school => (
                                  <div key={school.id} className="flex items-center justify-between p-2 bg-background rounded-lg border border-border/30">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-sm font-medium">{renderSchoolDisplayName(school.name)}</span>
              ...
              {/* Growth Pathway (CollapsibleSection moved) */}
              ...
              {/* Build Your Own Greenhouse Cluster CTA (moved into Card 1) */}
          ...
          <CollapsibleCard title="🏗️ Greenhouse Initiative" accent="blue" badge="1,200 Greenhouses" defaultOpen={false}>
            <div className="space-y-6 pt-2">
              ...
              {/* Construction Phase Jobs section (use existing Construction Phase Jobs content) */}
              {expandedJobs && expandedJobs.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.31 }}>
                  <Card className="glass-panel" data-testid="card-expanded-jobs">
                    ...
                    <CardContent>
                      ...
                      {/* Construction Phase Jobs (detailed breakdown) */}
                      <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-950/30 rounded-xl border border-blue-200 dark:border-blue-800" data-testid="section-construction-jobs">
                        <div className="flex items-center gap-2 mb-4">
                          ...
                          <Badge variant="outline" className="ml-auto bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-700">Skilled Trades</Badge>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                          {expandedJobs.map((job) => (
                            <div key={job.id} className="p-3 bg-white/70 dark:bg-black/30 rounded-lg border border-blue-100 dark:border-blue-800/50" data-testid={`card-construction-${job.scale}`}>
                              ...
                              <p className="text-[10px] text-blue-500 mt-1" data-testid={`text-construction-duration-${job.scale}`}>{job.constructionDurationYears || "TBD"}</p>
              ...
              {/* END Construction Phase Jobs */}

              {/* Specialty Crops card */}
              <div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-emerald-700 dark:text-emerald-400 mt-1">
                        Zero risk of sulfide acid mine drainage into the Boundary Waters. Instead, {miningAlternatives?.reduce((sum, a) => sum + (a.co2Sequestered || 0), 0).toLocaleString()} metric tons of CO2 emissions avoided annually, {(miningAlternatives?.reduce((sum, a) => sum + (a.localFoodProduction || 0), 0) / 1000000).toFixed(1)} million lbs of specialty produce for schools, and 2.4x economic multiplier keeping dollars in these communities.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <CollapsibleCard title="🪶 Tribal Partnerships" accent="amber" badge="Leech Lake Nation" defaultOpen={false}>
            <div className="space-y-6 pt-2">
                        ...
                        {tribalPartnerships.map((partnership) => (
                          <div key={partnership.id} className="p-4 bg-muted/30 rounded-xl border border-border/50" data-testid={`tribal-${partnership.id}`}>
                            ...
                            <div className="p-3 bg-muted/30 rounded-lg mb-3">
                              <p className="text-xs font-medium text-muted-foreground mb-1 flex items-center gap-1"><ShieldCheck className="h-3 w-3" /> Governance & Fraud Protection</p>
                            ...
                            {partnership.complementaryProjects && (
                              <div className="p-3 bg-emerald-50/50 dark:bg-emerald-950/20 rounded-lg border border-emerald-100 dark:border-emerald-900/30">
                                <p className="text-xs font-medium text-emerald-700 dark:text-emerald-400 mb-1 flex items-center gap-1"><Leaf className="h-3 w-3" /> Complementary Projects</p>
              ...
              {/* Excess Revenue Allocation: Leech Lake Tribal Programs ONLY (moved here) */}
              <div className="mt-4">
                <div className="mt-4 p-4 bg-gradient-to-r from-amber-50 to-emerald-50 dark:from-amber-950/30 dark:to-emerald-950/30 rounded-xl border border-amber-200 dark:border-amber-800">
                  <h4 className="font-semibold text-foreground mb-3 flex items-center gap-2">
                    <Sparkles className="h-4 w-4 text-amber-600" /> Excess Revenue Allocation (highlight)
                  </h4>
          ...
          {/* CARD 4: Endowment & Financials */}
          <CollapsibleCard title="💰 Endowment & Financials" accent="amber" badge="$5B Target" defaultOpen={false}>
            <div className="space-y-6 pt-2">
              {/* Endowment Funding Sources (fundingSources card) */}
              {fundingSources && fundingSources.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.495 }}>
                  <Card className="glass-panel" data-testid="card-funding">
                    ...
                    <CardContent>
                      ...
                      {/* Summary stat cards (Total Potential Revenue, Core Endowment Target, Excess Allocation, Year 1 Draw) */}
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            {/* Excess Revenue Allocation breakdown (Boundary Waters + Distribution Infrastructure ONLY here) */}
                            <div className="mt-4 p-4 bg-gradient-to-r from-amber-50 to-emerald-50 dark:from-amber-950/30 dark:to-emerald-950/30 rounded-xl border border-amber-200 dark:border-amber-800">
                              <h4 className="font-semibold text-foreground mb-3 flex items-center gap-2">
                                <Sparkles className="h-4 w-4 text-amber-600" /> Excess Revenue Allocation ({formatLargeNumber(excessAllocation)} over target)
                              </h4>
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div className="p-3 bg-white/50 dark:bg-black/20 rounded-lg">
                                ...
                                {/* Note: Leech Lake Tribal Programs has been moved to Card 3 */}
                              </div>
                            </div>
                          </>
                        );
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.497 }}>
                <FundingCalculator />
              </motion.div>

              {/* Perpetual Endowment StatsCard (keep as-is) */}
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.475 }}>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {/* Financial Projection Engine v4.1 (financials card) */}
              ...
              {/* Financial Resilience / Stress Tests section (CollapsibleSection) */}
          ...
          {/* CARD 5: Twin Metals Alternative */}
          <CollapsibleCard title="⛏️ Twin Metals Alternative" accent="rose" badge="Boundary Waters" defaultOpen={false}>
            <div className="space-y-6 pt-2">
              {/* Boundary Waters Alternative content (Twin Metals Dilemma + Foreign Ownership + lists) */}
              {miningAlternatives && miningAlternatives.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.4955 }}>
                  <Card className="glass-panel border-2 border-emerald-200 dark:border-emerald-800" data-testid="card-mining-alternative">
                    ...
                    <CardContent>
                      ...
                      {/* Foreign Ownership & What Leaves MN / What MN Gets */}
                      <div className="mb-6 p-4 bg-red-50 dark:bg-red-950/30 rounded-xl border border-red-200 dark:border-red-800">
                        ...
                        {/* Town-by-town community comparison cards (miningAlternatives mapping) */}
                            ...
                            <div key={alt.id} className="p-4 bg-muted/30 rounded-xl border border-border/50" data-testid={`mining-alt-${alt.id}`}>
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="pt-2 border-t border-border/30 space-y-1">
                                  ...
                                  {alt.specialtyCrops && <p className="text-xs text-purple-700 dark:text-purple-400 font-medium mt-1">{alt.specialtyCrops}</p>}
                                </div>
                              </div>
                            </div>
                          ))}
                        ...
                        {/* Summary comparison stats row & Gaia Commons Alternative table */}
                        ...
                        {/* Gaia Commons Alternative 100% Minnesota-Owned comparison table */}
                        <div className="mt-6 p-4 bg-emerald-50 dark:bg-emerald-950/30 rounded-xl border border-emerald-200 dark:border-emerald-800">
                          ...
                          {/* Labor Transition Program section */}
                          {laborTransition && laborTransition.length > 0 && (
                            <div className="mt-6">
                              <Card className="glass-panel" data-testid="card-labor-transition">
                                ...
                                <CardContent>
                                  <p className="text-sm text-muted-foreground mb-4">125% income guarantee for 3 years + $45K retraining. Workers choose timing and pathway.</p>
                                  <div className="space-y-3">
          ...
          {/* CARD 6: Climate Impact */}
          <CollapsibleCard title="🌍 Climate Impact" accent="violet" badge="NASA/NOAA Validated" defaultOpen={false}>
            <div className="space-y-6 pt-2">
              {/* Job Creation & Economic Impact (Job Creation card moved here) */}
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
                <Card className="glass-panel h-full" data-testid="card-jobs-climate">
                  <CardHeader className="flex flex-row items-center gap-2 space-y-0 pb-4">
                    <Briefcase className="h-5 w-5 text-primary" />
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-4 bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-950/30 dark:to-indigo-950/30 rounded-xl border border-purple-100 dark:border-purple-900/50">
                            ...
                            <p className="text-2xl font-bold text-purple-700 dark:text-purple-300" data-testid="text-jobs-total">{formatNumber(currentJobs.totalJobs)}</p>
                          </div>
                          <div className="p-4 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 rounded-xl border border-amber-100 dark:border-amber-900/50">
                            ...
                            <p className="text-2xl font-bold text-amber-700 dark:text-amber-300" data-testid="text-jobs-impact">{formatLargeNumber(currentJobs.economicImpact)}</p>
                          </div>
                        </div>
              ...
              {globalRegenerationSummary && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.53 }}>
              ...
              {tieredCarbonPricing && tieredCarbonPricing.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.54 }}>
                  <Card className="glass-panel" data-testid="card-tiered-carbon">
                    ...
                    <CardContent>
                      <p className="text-sm text-muted-foreground mb-4">Progressive carbon tax structure: Small businesses pay $25/ton, mega-polluters face $200/ton + exponential penalties.</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {tieredCarbonPricing.map((tier) => (
                          <div key={tier.id} className={`p-4 rounded-lg border ${tier.carbonTaxRate <= 25 ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/50' : tier.carbonTaxRate <= 75 ? 'bg-blue-50/50 dark:bg-blue-950/20 border-blue-200 dark:border-blue-900/50' : tier.carbonTaxRate <= 150 ? 'bg-amber-50/50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/50' : 'bg-red-50/50 dark:bg-red-950/20 border-red-200 dark:border-red-900/50'}`} data-testid={`carbon-tier-${tier.id}`}>
                            <div className="flex items-center justify-between mb-2">
              ...
              {politicalCoalitionData && politicalCoalitionData.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.58 }}>
              ...
              {/* Environmental Impact (moved here as well) */}
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
                <Card className="glass-panel h-full" data-testid="card-environmental-climate">
                  <CardHeader className="flex flex-row items-center gap-2 space-y-0 pb-4">
              ...
              {/* Model Calibration & Validation (modelMaturity, calibrationTargets, planetaryBoundaries, historicalClimateData) */}
              {(modelMaturity && modelMaturity.length > 0) && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.59 }}>
                  <Card className="glass-panel bg-gradient-to-r from-blue-50/50 to-purple-50/50 dark:from-blue-950/20 dark:to-purple-950/20" data-testid="card-model-validation">
                    ...
                    <CardContent>
                      <p className="text-sm text-muted-foreground mb-6">All projections are grounded in empirical data from NASA, NOAA, IEA, World Bank, and peer-reviewed science. Our models are continuously calibrated against real-world observations.</p>

                      {modelMaturity && modelMaturity.length > 0 && (
                        <div className="mb-6">
                          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                            <Gauge className="h-4 w-4 text-primary" /> Model Maturity Levels
                          </h4>
                            ...
                            {modelMaturity.map((m) => (
                              <div key={m.id} className="p-3 bg-background/60 rounded-lg border border-border/50" data-testid={`maturity-${m.id}`}>
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <Badge variant="outline" className={`text-xs ${m.maturityLevel === 'validated' ? 'border-emerald-500 text-emerald-700 dark:text-emerald-400' : m.maturityLevel === 'calibrated' ? 'border-blue-500 text-blue-700 dark:text-blue-400' : 'border-amber-500 text-amber-700 dark:text-amber-400'}`}>
                      ...
                      {calibrationTargets && calibrationTargets.length > 0 && (
                        <div className="mb-6">
                          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                            <Target className="h-4 w-4 text-primary" /> Calibration Accuracy ({calibrationTargets.filter(t => t.status === 'passed').length}/{calibrationTargets.length} Passed)
                          </h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {calibrationTargets.map((t) => (
                              <div key={t.id} className="p-3 bg-background/60 rounded-lg border border-border/50" data-testid={`calibration-${t.id}`}>
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <Badge variant="outline" className={`text-xs ${t.status === 'passed' ? 'border-emerald-500 text-emerald-700 dark:text-emerald-400' : t.status === 'warning' ? 'border-amber-500 text-amber-700 dark:text-amber-400' : 'border-red-500 text-red-700 dark:text-red-400'}`}>
                      ...
                      {planetaryBoundaries && planetaryBoundaries.length > 0 && (
                        <div>
                          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                            <Globe className="h-4 w-4 text-primary" /> Planetary Boundaries Status (Steffen et al. 2015, Richardson et al. 2023)
                          </h4>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {planetaryBoundaries.map((b) => (
                              <div key={b.id} className="p-3 bg-background/60 rounded-lg border border-border/50" data-testid={`boundary-${b.id}`}>
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <Badge variant="outline" className={`text-xs ${b.status === 'safe' ? 'border-emerald-500 text-emerald-700 dark:text-emerald-400' : b.status === 'caution' ? 'border-amber-500 text-amber-700 dark:text-amber-400' : 'border-red-500 text-red-700 dark:text-red-400'}`}>
                                ...
                                <div className="mb-2">
                                  ...
                                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                                    <div
                                      className={`h-full rounded-full ${b.status === 'safe' ? 'bg-emerald-500' : b.status === 'caution' ? 'bg-amber-500' : 'bg-red-500'}`}
                      ...
                      {historicalClimateData && historicalClimateData.length > 0 && (
                        <div className="mt-6">
                          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2">
                            <Thermometer className="h-4 w-4 text-primary" /> Historical Validation Data (2015-2024)
                          </h4>
              ...
              {/* Advanced Modeling (Monte Carlo, scenarioComparisons, optimizationParams, sensitivityAnalysis) */}
              {(monteCarloSimulations && monteCarloSimulations.length > 0) && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.60 }}>
                  <Card className="glass-panel bg-gradient-to-r from-purple-50/50 to-indigo-50/50 dark:from-purple-950/20 dark:to-indigo-950/20" data-testid="card-advanced-modeling">
                    ...
                    <CardContent>
                      <p className="text-sm text-muted-foreground mb-6">
                        Probabilistic simulations with confidence intervals, scenario comparisons, and optimization analysis for robust decision-making under uncertainty.
                      </p>

                      {/* Monte Carlo */}
                      <div className="mb-6">
                        <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2"><BarChart3 className="h-4 w-4 text-primary" /> Monte Carlo Uncertainty Analysis (95% Confidence)</h4>
                          ...
                          {monteCarloSimulations.map((mc) => (
                            <div key={mc.id} className="p-3 bg-background/60 rounded-lg border border-border/50" data-testid={`monte-carlo-${mc.id}`}>
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <Badge variant="outline" className="text-xs border-purple-500 text-purple-700 dark:text-purple-400">{mc.scale}</Badge>
                              </div>
                              <div className="mb-2">
                                <div className="text-lg font-bold text-foreground">
                                  {mc.unit === 'USD' || mc.unit === 'USD/year' ? formatLargeNumber(mc.p50Value) : formatNumber(mc.p50Value)} <span className="text-xs font-normal text-muted-foreground">{mc.unit !== 'USD' && mc.unit !== 'USD/year' ? mc.unit : ''}</span>
                                </div>
                      ...
                      {scenarioComparisons && scenarioComparisons.length > 0 && (
                        <div className="mb-6">
                          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2"><GitCompare className="h-4 w-4 text-primary" /> Scenario Comparison (Baseline / Optimistic / Conservative)</h4>
                      ...
                      {optimizationParams && optimizationParams.length > 0 && (
                        <div className="mb-6">
                          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2"><SlidersHorizontal className="h-4 w-4 text-primary" /> Optimization Targets</h4>
                            ...
                            {optimizationParams.map((o) => (
                              <div key={o.id} className="p-3 bg-background/60 rounded-lg border border-border/50" data-testid={`optimization-${o.id}`}>
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <Badge variant="outline" className={`text-xs ${o.feasibility === 'achievable' ? 'border-emerald-500 text-emerald-700 dark:text-emerald-400' : 'border-amber-500 text-amber-700 dark:text-amber-400'}`}>{o.feasibility}</Badge>
                                </div>
                                <div className="flex items-center gap-2 mb-2">
                                ...
                                <div className="h-2 bg-muted rounded-full overflow-hidden mb-1">
                                  <div className="h-full bg-gradient-to-r from-blue-500 to-emerald-500 rounded-full" style={{ width: `${Math.min(100, (o.optimalValue / o.targetValue) * 100)}%` }} />
                                </div>
                      ...
                      {sensitivityAnalysis && sensitivityAnalysis.length > 0 && (
                        <div>
                          <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-2"><ArrowUpDown className="h-4 w-4 text-primary" /> Sensitivity Analysis (Parameter Impact Ranking)</h4>
                          <div className="space-y-2">
                            {sensitivityAnalysis.map((s) => (
                              <div key={s.id} className="flex items-center gap-3 p-2 bg-background/60 rounded-lg border border-border/50" data-testid={`sensitivity-${s.id}`}>
                                <div className="flex items-center justify-center w-8 h-8 rounded-full bg-primary/10 text-primary font-bold text-sm">{s.rank}</div>
                                ...
                                <div className="w-24">
                                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                                    <div className={`h-full rounded-full ${s.elasticity > 0 ? 'bg-emerald-500' : 'bg-red-500'}`} style={{ width: `${Math.min(100, Math.abs(s.elasticity) * 50)}%` }} />
                                  </div>
                                </div>
                              </div>
                            ))}
          ...
          <CollapsibleCard title="⚖️ Legal & Ballot Drive" accent="cyan" badge="2026 Ballot Initiative" defaultOpen={false}>
            <div className="space-y-6 pt-2">
              ...
              {transparencyFeatures && transparencyFeatures.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.496 }}>
                  <Card className="glass-panel" data-testid="card-transparency">
                    ...
                    <CardContent>
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-3 flex items-center gap-2"><Eye className="h-3 w-3" /> Transparency Features</p>
                          <div className="space-y-3">
                        ...
                        {accountabilityMechanisms && accountabilityMechanisms.length > 0 && (
                          <div>
                            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-3 flex items-center gap-2"><ClipboardCheck className="h-3 w-3" /> Accountability Mechanisms</p>
                            <div className="space-y-3">
              ...
              {/* Political Roadmap (8 districts) */}
              {politicalRoadmap && politicalRoadmap.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.51 }}>
                    ...
                    <CardContent>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {politicalRoadmap.map((district) => (
                          <div
                            key={district.id}
                            className={`p-4 rounded-lg border ${district.supportLevel === 'Strong YES' ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-900/50' : district.supportLevel === 'Competitive' ? 'bg-amber-50/50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900/50' : 'bg-red-50/50 dark:bg-red-950/20 border-red-200 dark:border-red-900/50'}`}
                          ...
                          >
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <Badge
                                variant="outline"
                                className={`text-xs ${district.supportLevel === 'Strong YES' ? 'border-emerald-500 text-emerald-700 dark:text-emerald-400' : district.supportLevel === 'Competitive' ? 'border-amber-500 text-amber-700 dark:text-amber-400' : 'border-red-500 text-red-700 dark:text-red-400'}`}
                              >
                                {district.supportPct}
                              </Badge>
                            </div>
                            <p className={`text-sm font-medium mb-2 ${district.supportLevel === 'Strong YES' ? 'text-emerald-700 dark:text-emerald-400' : district.supportLevel === 'Competitive' ? 'text-amber-700 dark:text-amber-400' : 'text-red-700 dark:text-red-400'}`}>{district.supportLevel}</p>
                            ...
                            <div key={tier}>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {tierPartners.map(partner => (
                                  ...
                                  >
                                    {partner.name}
                                    {partner.memberCount && partner.memberCount > 0 && (<span className="ml-1 opacity-70">({formatNumber(partner.memberCount)})</span>)}
                                  </Badge>
                                ))}
                              </div>
                            </div>
              ...
              {/* Ballot Initiative Presentation button (moved here) */}
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.19 }}>
                <Card className="glass-panel border-purple-500/20 bg-gradient-to-r from-purple-500/5 to-indigo-500/5" data-testid="card-ballot-deck-cta-2">
                  <CardContent className="py-6">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground">View the complete "One Vote, Forever Fed" deck for community presentations</p>
                        </div>
                      </div>
                      <Link href="/ballot-presentation">
                        <Button size="lg" variant="outline" className="gap-2 border-purple-500/30" data-testid="button-goto-presentation-2">
              ...
              {/* Governance principles cards (duplicate footer principles but kept inside card as requested) */}
              ...
              {/* Share & Spread the Word (SocialSharing) */}
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.75 }}>
                <SocialSharing />
              </motion.div>

              {/* Stay Updated email subscribe (EngagementFooter or appropriate subscribe content) */}
              <div>
                <EngagementFooter />
              </div>
            </div>
          ...
          {/* FOOTER — must remain OUTSIDE all CollapsibleCards */}
          ...
          {/* Social Sharing & Engagement (outside cards) */}
